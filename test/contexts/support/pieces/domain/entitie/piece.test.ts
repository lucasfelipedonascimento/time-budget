...
test("deve lançar erro ao passar peça sem campo obrigatório", () => {
  expect(() => {
    new Piece("", pieceMock.unit_price, pieceMock.quantity, pieceMock.id);
  }).toThrowError("O nome é obrigatório");

  expect(() => {
    new Piece(pieceMock.name, -1, pieceMock.quantity, pieceMock.id);
  }).toThrowError("Preço unitário não pode ser negativo");

  expect(() => {
    new Piece(pieceMock.name, pieceMock.unit_price, 0, pieceMock.id);
